function [meanrecipvif, vifs, design_struct] = generate_jittered_er_design_MainTask(varargin)
...
event2duration = 0;    % duration of resp
trialtypes = 2;        % 
trialspertype = 80;
ISI2isconstant = 0;    % ITI is constant (as opposed to jittered).
...
ISImin = 1.7;           % Constraints: Psychological (can subjects process cue) and statistical (longer = less BOLD nonlinearity, which is difficult to model).
ISImean = 4;            % For 'exponential' only.  Includes ISImin.  There is an optimal empirical value -- longer is better for deconvolution/FIR, but we also need to fit within total scan time constraints.
...
design_struct = struct('ons', [], 'TR', TR, 'event1duration', event1duration, 'event2duration', event1duration, 'trialtypes', trialtypes, 'trialspertype', trialspertype, 'scanlength', ceil(scanlength));
